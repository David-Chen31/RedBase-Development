...
string QueryOptimizer::GetRelationName(const RelAttr &attr, const PlanNode *node) {
    ...
    if (auto selectNode = dynamic_cast<const SelectNode*>(node)) {
        if (selectNode->childNode) {
            return GetRelationName(attr, selectNode->childNode.get());
        }
    }
    
    // 3.3 投影节点 - 递归到子节点获取
    if (auto projectNode = dynamic_cast<const ProjectNode*>(node)) {
        if (projectNode->childNode) {
            return GetRelationName(attr, projectNode->childNode.get());
        }
    }
    
    // 3.4 连接节点 - 从输出属性中查找
    // 连接节点包含多个表，需要通过属性名匹配
    if (dynamic_cast<const JoinNode*>(node)) {
        if (attr.attrName) {
            for (const auto &outputAttr : node->outputAttrs) {
                if (strcmp(outputAttr.attrName, attr.attrName) == 0) {
                    return string(outputAttr.relName);
                }
            }
        }
    }
    ...
    // 4. 通用方法：从节点的输出属性中查找
    // 适用于所有节点类型
    if (attr.attrName && !node->outputAttrs.empty()) {
        for (const auto &outputAttr : node->outputAttrs) {
            if (strcmp(outputAttr.attrName, attr.attrName) == 0) {
                // 找到匹配的属性，返回其所属的关系名
                if (strlen(outputAttr.relName) > 0) {
                    return string(outputAttr.relName);
                }
            }
        }
