...
ParsedSQL SQLParser::ParseSelect(const vector<string> &tokens) {
    ...
    for (int i = 4; i < tokens.size(); i++) {
        if (ToUpper(tokens[i]) == "WHERE" && i + 3 < tokens.size()) {
            ...
            // 推断右值类型
            string rhsStr = tokens[i+3];
            AttrType valType = INT;
            if ((rhsStr.front() == '\'' && rhsStr.back() == '\'') ||
                (rhsStr.front() == '"' && rhsStr.back() == '"')) {
                valType = STRING;
                rhsStr = rhsStr.substr(1, rhsStr.length() - 2);
            } else if (rhsStr.find('.') != string::npos) {
                valType = FLOAT;
            }
            cond.rhsValue = StringToValue(rhsStr, valType);
